...
const activeChatMap = new Map<string, string>();
...
const broadcastPresence = async (io: Server, userId: string, isOnline: boolean) => {
...
export const initSocketServer = (server: HTTPServer) => {
  ...
  io.use((socket, next) => {
    const tokenHeader = socket.handshake.headers.authorization?.startsWith("Bearer ")
      ...
      : undefined;
    const token = (socket.handshake.auth?.token as string | undefined) ?? tokenHeader;
    ...
    try {
      ...
      next(createHttpError(401, (error as Error).message));
    }
  });
            ...
            if (recipientIds.length) {
              ...
              if (sender) {
                await sendMessageNotification(
                  recipientIds,
                  message as any,
                  chat.type === "group" ? chat.groupName ?? sender.name : sender.name
                );
              }
            }
          }
