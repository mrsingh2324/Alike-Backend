...
export const getMessages = async (
  ...
  const filters: Record<string, unknown> = { chatId };
  if (options.before) {
...
const ensureNotBlockedForMessage = async (chatId: string, senderId: string) => {
...
export const markMessagesDelivered = async (chatId: string, userId: string) => {
  await ensureChatMembership(chatId, userId);
  await MessageModel.updateMany(
    { chatId, senderId: { $ne: userId }, status: MessageStatus.SENT },
...
export const markMessagesRead = async (chatId: string, userId: string) => {
  await ensureChatMembership(chatId, userId);
  await MessageModel.updateMany(
    { chatId, senderId: { $ne: userId }, status: { $ne: MessageStatus.READ } },
