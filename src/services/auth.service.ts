...
const upsertOtp = async (channel: OTPChannel, target: string, code: string) => {
...
const sendPhoneOtp = async (phone: string, code: string): Promise<void> => {
...
const getOrCreateUser = async ({
  ...
  name,
  profilePicUrl
}: Pick<VerifyParams, "email" | "phone" | "name" | "profilePicUrl">) => {
  if (!name) {
...
const consumeOtp = async ({ channel, target, otp }: { channel: OTPChannel; target: string; otp: string }) => {
...
export const verifyOtp = async (params: VerifyParams) => {
