...
export const ensureChatMembership = async (chatId, userId) => {
...
export const createOrGetSingleChat = async (userId, targetUserId) => {
...
export const createGroupChat = async (creatorId, payload) => {
    ...
    await ChatParticipantModel.insertMany(memberIds.map((memberId) => ({
        chatId: chat.id,
        userId: memberId,
...
export const addMembers = async (chatId, adminId, memberIds) => {
    ...
    if (!newMembers.length)
        return;
    await ChatParticipantModel.insertMany(newMembers.map((userId) => ({ chatId, userId, role: "member" })));
    chat.members = Array.from(new Set([...chat.members.map((m) => m.toString()), ...newMembers])).map((id) => new Types.ObjectId(id));
    await chat.save();
};
export const removeMembers = async (chatId, adminId, memberIds) => {
    const adminParticipant = await ensureChatMembership(chatId, adminId);
    if (adminParticipant.role !== "admin") {
...
export const updateGroupDetails = async (chatId, adminId, payload) => {
    const adminParticipant = await ensureChatMembership(chatId, adminId);
    if (adminParticipant.role !== "admin") {
...
export const makeAdmin = async (chatId, adminId, memberId) => {
    const adminParticipant = await ensureChatMembership(chatId, adminId);
    if (adminParticipant.role !== "admin") {
...
export const incrementUnreadForParticipants = async (chatId, senderId) => {
    await ChatParticipantModel.updateMany({ chatId, userId: { $ne: senderId } }, { $inc: { unreadCount: 1 } });
};
